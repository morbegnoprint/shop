...
export class FullRelationControl extends React.Component {
        ...
        if (queryHits !== prevProps.queryHits && queryHits.get(forID)) {
            ...
            if (value) {
                ...
                listValue.forEach(val => {
                    const hit = hits.find(
                        i => this.parseNestedFields(i.data) === val
                    );
                    if (hit) {
                        onChange(value, {
                            [field.get("name")]: {
    ...
    parseNestedFields = targetObject => JSON.stringify(targetObject);

    parseHitOptions = hits => {
        const { field } = this.props;
        const displayField =
            field.get("displayFields") || field.get("valueField");
        ...
        return hits.map(hit => {
            ...
            if (List.isList(displayField)) {
                labelReturn = displayField
                    .toJS()
                    .map(key => this.parseNestedFields(hit.data))
                    .join(" ");
            } else {
                labelReturn = this.parseNestedFields(hit.data);
            }
            return {
                data: hit.data,
                value: this.parseNestedFields(hit.data),
                label: labelReturn
            };
        });
    };
